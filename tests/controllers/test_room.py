...
from ibg.models.room import RoomType, RoomStatus, RoomCreate, RoomJoin, RoomLeave
...
async def test_create_room_with_valid_owner_and_password(
    ...
    room_user_link = session.exec(
        select(RoomUserLink).where(RoomUserLink.room_id == room.id)
    ).one()
...
async def test_join_then_leave_and_join_again_a_room(
    ...
    _ = await room_controller.leave_room(
        RoomLeave(room_id=room.id, user_id=user_to_join.id)
    )
    updated_room = await room_controller.join_room(
        RoomJoin(room_id=room.id, user_id=user_to_join.id, password=room.password)
    )
...
@pytest.mark.asyncio
async def test_join_room_twice(
    user_controller: UserController, room_controller: RoomController, faker: Faker
):
    # Arrange
    user_create = UserCreate(
...
async def test_leave_room(
    ...
    _ = await room_controller.join_room(
        RoomJoin(room_id=room.id, user_id=user.id, password=room.password)
    )
    leaved_room = await room_controller.leave_room(
        RoomLeave(room_id=room.id, user_id=user.id)
    )
...
async def test_leave_non_existent_room(room_controller: RoomController, faker: Faker):
    with pytest.raises(RoomNotFoundError):
        _ = await room_controller.leave_room(
            RoomLeave(room_id=faker.uuid4(), user_id=faker.uuid4())
        )


@pytest.mark.asyncio
async def test_leave_twice(
    user_controller: UserController, room_controller: RoomController, faker: Faker
):
    owner = await user_controller.create_user(
        UserCreate(
...
async def test_leave_room_non_existent_user(
    ...
    with pytest.raises(UserNotFoundError):
        _ = await room_controller.leave_room(
            RoomLeave(room_id=room.id, user_id=faker.uuid4())
        )
...
async def test_leave_room_non_existent_room(
    ...
    with pytest.raises(RoomNotFoundError):
        _ = await room_controller.leave_room(
            RoomLeave(room_id=faker.uuid4(), user_id=user.id)
        )
...
async def test_leave_room_owner(
    ...
    empty_room = await room_controller.leave_room(
        RoomLeave(room_id=room.id, user_id=owner.id)
    )
    room_user_link = session.exec(
        select(RoomUserLink).where(RoomUserLink.room_id == room.id)
    ).one()
...
@pytest.mark.asyncio
async def test_leave_room_user_not_in(
    user_controller: UserController, room_controller: RoomController, faker: Faker
):
    owner = await user_controller.create_user(
        UserCreate(
