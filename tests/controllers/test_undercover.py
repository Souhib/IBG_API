...
from ibg.models.undercover import WordCreate, Word, WordUpdate, TermPair
...
async def test_create_term_pair_raises_exception_if_term_pair_already_exists(
    ...
    with pytest.raises(
        TermPairAlreadyExistsError, match="Term pair .* - .* already exists"
    ):
...
@pytest.mark.asyncio
async def test_delete_term_pair(
    undercover_controller: UndercoverController, faker: Faker
):
    word1 = await undercover_controller.create_word(
        WordCreate(
