...
from ibg.models.room import RoomType, RoomStatus
from ibg.models.models import Room, User, Game
from tests.conftest import get_test_app
from ibg.dependencies import get_room_controller


@pytest.mark.asyncio
async def test_get_rooms(room_controller: RoomController, faker: Faker):
    app, client = get_test_app()
...
@pytest.mark.asyncio
async def test_get_room(room_controller: RoomController, faker: Faker):
    app, client = get_test_app()
...
@pytest.mark.asyncio
async def test_get_room_when_room_does_not_exist(room_controller: RoomController):
    app, client = get_test_app()

    room_id = uuid4()

    def _mock_get_room_by_id():
        room_controller.get_room_by_id = AsyncMock(
            side_effect=RoomNotFoundError(room_id=room_id)
        )
        return room_controller
...
async def test_get_room_with_multiple_users_and_multiple_games(
    room_controller: RoomController, faker: Faker
):
    app, client = get_test_app()
...
@pytest.mark.asyncio
async def test_create_room(room_controller: RoomController, faker: Faker):
    app, client = get_test_app()
...
@pytest.mark.asyncio
async def test_join_room(room_controller: RoomController, faker: Faker):
    app, client = get_test_app()
...
@pytest.mark.asyncio
async def test_join_room_when_room_does_not_exist(room_controller: RoomController):
    app, client = get_test_app()
    ...
    def _mock_join_room():
        room_controller.join_room = AsyncMock(
            side_effect=RoomNotFoundError(room_id=room_id)
        )
        return room_controller
...
@pytest.mark.asyncio
async def test_join_room_when_user_already_in_room(room_controller: RoomController):
    app, client = get_test_app()
    ...
    def _mock_join_room():
        room_controller.join_room = AsyncMock(
            side_effect=UserAlreadyInRoomError(user_id=user_id, room_id=room_id)
        )
        return room_controller
...
@pytest.mark.asyncio
async def test_leave_room(room_controller: RoomController, faker: Faker):
    app, client = get_test_app()
...
@pytest.mark.asyncio
async def test_leave_room_when_room_does_not_exist(room_controller: RoomController):
    app, client = get_test_app()
    ...
    def _mock_leave_room():
        room_controller.leave_room = AsyncMock(
            side_effect=RoomNotFoundError(room_id=room_id)
        )
        return room_controller
...
@pytest.mark.asyncio
async def test_leave_room_when_user_not_in_room(room_controller: RoomController):
    app, client = get_test_app()
    ...
    def _mock_leave_room():
        room_controller.leave_room = AsyncMock(
            side_effect=UserNotInRoomError(user_id=user_id, room_id=room_id)
        )
        return room_controller
...
@pytest.mark.asyncio
async def test_leave_room_when_user_not_found(room_controller: RoomController):
    app, client = get_test_app()
...
@pytest.mark.asyncio
async def test_delete_room(room_controller: RoomController):
    app, client = get_test_app()
...
@pytest.mark.asyncio
async def test_join_room_wrong_password(room_controller: RoomController):
    app, client = get_test_app()
    ...
    def _mock_join_room():
        room_controller.join_room = AsyncMock(
            side_effect=WrongRoomPasswordError(room_id=room_id)
        )
        return room_controller
